    ...
    return {
      ...
      errors: []
          ...
          if (this.lat.length > 0 && this.lng.length > 0) {
           ...
           //axios.get('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + this.lat + ',' + this.lng + '&key=AIzaSyBSUwF0_JfS0CZybBeAI-aqzz04_e67jDg')
        const res = await fetch('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + this.lat + ',' + this.lng + '&key=AIzaSyBSUwF0_JfS0CZybBeAI-aqzz04_e67jDg')
        
const dataDB = await res.json()

console.log(dataDB.results[0].formatted_address)
this.address = dataDB.results[0].formatted_address
        /*.then(response => {
